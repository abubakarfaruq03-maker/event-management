...
export default function Main({ events, setEvents }: MainProps) {
  ...
  return (
    <div className="relative bg-background min-h-screen transition-colors duration-500 font-poppins antialiased overflow-x-hidden">
      ...
      <div className={`fixed top-10 left-1/2 -translate-x-1/2 z-[100] pointer-events-none transition-all duration-500 ease-out ${showSuccess ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-10"
        }`}>
      ...
      {/* --- SIDEBAR PANEL --- */}
      <aside
        className={`fixed top-0 right-0 h-full w-full md:w-[450px] bg-card border-l border-border-line/50 z-50 shadow-2xl transition-transform duration-500 ease-in-out ${isSidebarOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
      ...
      <main className="md:mx-20 mx-10 pb-20 mt-10 sm:mt-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {events.length > 0 ? (
            events.slice()
              .sort((a, b) => new Date(`${a.date}T${a.time}:00`).getTime()
